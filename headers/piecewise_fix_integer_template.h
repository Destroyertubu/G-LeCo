...
namespace Codecset
{
    ...
    public:
        ...
        uint8_t *encodeArray8_int(const T *data, const size_t length, uint8_t *res, size_t index)
        {
            uint8_t *out = res;
            ...
            // if (index == 0)
            //     outfile.open("deltas.txt", std::ios::out);
        ...
        T *decodeArray8(const uint8_t *in, const size_t length, T *out, size_t nvalue)
        {
            ...
            if (maxerror)
            {
                ...
                read_all_bit_fix<T>(tmpin, 0, 0, length, maxerror, theta1, theta0, res, nvalue);
            }
            else
            {
