...
#include <cstdint>  // For uint64_t (if needed)
...
namespace Codecset {
    ...
    public:
        ...
        uint8_t* encodeArray8_int(T* in, const size_t length, uint8_t* res, size_t nvalue) {
            ...
            // todo: segment_index, new_segmentsize(signvec), segment_length, merge_both_direction(new_segment_index, new_segment_length), newsegment(signvec, delta_final),alex_build_vec
            ...
            uint64_t tmp_tbt = total_byte_total;
            ...
            // outfile << "i = " << nvalue << ", total_byte = " << total_byte_total - tmp_tbt << ", segment_index size = " << segment_index.size() - 1 << std::endl;
        ...
        void merge_both_direction(int nvalue, int length) {
            ...
            while (segment_num < total_segments) {
                ...
                // if (nvalue == 0)
                //     std::cout << "last_merge_segment = " << last_merged_segment << " init_cost_front = " << init_cost_front << " merge_cost_front = " << merge_cost_front << " saved_cost_front = " << saved_cost_front << " init_cost_back = " << init_cost_back << " merge_cost_back = " << merge_cost_back << " saved_cost_back = " << saved_cost_back << " saved_cost = " << saved_cost << std::endl;
                if (saved_cost <= 0) {
                    // do not merge
                    new_segment_index.emplace_back(segment_index[segment_num]);
                    new_segment_length.emplace_back(segment_length[segment_num]);
            ...
            // file << "i = " << nvalue << ", total_byte = " << total_byte << std::endl;
        ...
        T* decodeArray8(const size_t length, T* out, size_t nvalue) {
            ...
            for (int i = 0;i < block_start_vec_total.size();i++) {
                ...
                if (maxerror) {
                    read_all_bit_fix<T>(tmpin, 0, 0, segment_length, maxerror, theta1, theta0, res, segment_index_total[i]);
