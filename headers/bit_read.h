...
template <typename T>
void read_all_bit_fix(const uint8_t* in, int start_byte, int start_index, int numbers, int l, double slope, double start_key, T* out, int index)
{
  int left = 0;
  uint128_t decode = 0;
  ...
  std::ofstream ofs("first_block.txt", std::ios::out);
  ...
  while (start <= end && writeind < numbers)
  {
    while (left >= l && writeind < numbers)
    {
      ...
      if (index == 10379)
        ofs << "decode_val = " << decode_val << std::endl;
